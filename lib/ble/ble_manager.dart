...
class BLEManager {
  ...
  final Map<String, Peripheral> _connectedPeripherals = {}; // peripheralId -> Peripheral
  final Map<String, List<GATTService>> _discoveredServices = {}; // peripheralId -> services
  ...
  void _setupSubscriptions() {
    ...
    _centralStateChangedSubscription = _central.stateChanged.listen((eventArgs) async {
    ...
    BlePeripheral.setWriteRequestCallback((String deviceId, String characteristicId, int offset, Uint8List? value) {
      if (value != null) {
        _handleReceivedChunk(deviceId, value);
      }
  ...
  Future<void> initialize() async {
    ...
    _central.authorize().then((isAuthorized) {
    ...
    await BlePeripheral.initialize().then((isAuthorized) {
  ...
  GATTCharacteristic? findCharacteristic(String peripheralId, UUID characteristicUUID) {
    final services = _discoveredServices[peripheralId];
    if (services == null) return null;
    
    for (final service in services) {
      for (final char in service.characteristics) {
        if (char.uuid.toString().toLowerCase() == characteristicUUID.toString().toLowerCase()) {
          return char;
        }
      }
    }
  ...
  Future<void> writeCharacteristic({
    ...
    GATTCharacteristicWriteType writeType = GATTCharacteristicWriteType.withResponse,
  }) async {
    final peripheralId = peripheral.uuid.toString();
    ...
    if (characteristic == null) {
      ...
      print('Warning: Characteristic not found in discovered services, creating mutable one');
    ...
    if (data.length <= maxDataPerChunk) {
      ...
      for (int i = 0; i < totalChunks; i++) {
        ...
        final chunk = _createChunk(chunkType, messageId, i, totalChunks, chunkData);
  ...
  Uint8List _createChunk(int type, int messageId, int chunkIndex, int totalChunks, Uint8List data) {
  ...
  void _handleReceivedChunk(String deviceId, Uint8List chunk) {
    if (chunk.length < 3) {
    ...
    print('Received chunk type=$chunkType, messageId=$messageId, data=${data.length} bytes');
    ...
    if (chunkType == _CHUNK_FIRST) {
      ...
      if (_chunkBuffers[deviceId]!.containsKey(messageId)) {
        ...
        print('Middle chunk appended (total: ${_chunkBuffers[deviceId]![messageId]!.length})');
      } else {
      ...
      if (_chunkBuffers[deviceId]!.containsKey(messageId)) {
        ...
        print('Message reassembled: ${completeMessage.length} bytes from ${chunks.length} chunks');
